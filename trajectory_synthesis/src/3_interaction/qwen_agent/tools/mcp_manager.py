...
class MCPManager:
    ...
    async def init_config_async(self, config: Dict):
        tools: list = []
        mcp_servers = config['mcpServers']
        tool_name_to_tool_info_map = {}
        if "tool_list" in config:
            for tool_info in config["tool_list"]:
                tool_name_to_tool_info_map[tool_info["name"]] = tool_info
        ...
        for server_name in mcp_servers:
            ...
            for tool in client.tools:
                ...
                """
                # Adaptive tool loading logic
                if tool.name not in tool_name_to_tool_info_map:
                    ...
                    tool_info = tool_name_to_tool_info_map[tool.name]
            ...
            if client.resources and "list_resources" not in tool_name_to_tool_info_map:
    ...
    async def connection_server(self, mcp_server_name, mcp_server):
        ...
        try:
            if 'url' in mcp_server:
                ...
                headers = mcp_server.get('headers', None)
                if mcp_server.get('type', 'sse') == 'streamable-http':
                    ...
                    self._streams_context = streamablehttp_client(
                        url=url, sse_read_timeout=datetime.timedelta(seconds=sse_read_timeout),headers=headers)
    ...
    async def execute_function(self, tool_name, tool_args: dict):
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.info(f"Session ping failed (attempt {attempt + 1}/{max_retries}), try reconnect: {e}")
                # Auto reconnect
                try:
                    ...
                    if self.client_id is not None:
                        ...
                        # Update self reference to use new client
                        self.session = new_client.session
                        self.exit_stack = new_client.exit_stack
                        logger.info(f"Reconnected successfully for client {self.client_id}")
                        continue  # Retry ping with new connection
                    else:
                        logger.info('Reconnect failed: client_id is None')
                        if attempt == max_retries - 1:
                            return 'Session reconnect exception: client_id is None'
                except Exception as e3:
                    logger.info(f'Reconnect exception (attempt {attempt + 1}): {type(e3)}, value: {repr(e3)}')
                    if attempt == max_retries - 1:
                        return f'Session reconnect exception after {max_retries} attempts: {e3}'
