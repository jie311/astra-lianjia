...
from qwen_agent.tools.base import ToolServiceError, MockToolWrapper
...
class Agent(ABC):
    ...
    def __init__(self,
        ...
        self.function_map = {}
        self.mock = False  # Default value, initialized before _init_tool, may be overwritten by mock tool in _init_tool
        if function_list:
    ...
    def _init_tool(self, tool: Union[str, Dict, BaseTool]):
        ...
        # Add mock tool loading logic, requires mock_tool field under each element of tools
        elif isinstance(tool, dict) and "mock_tool" in tool:
            self.mock = True
            tool_list = tool["tool_list"]
            server_description = tool.get("server_description", "")
            for mock_tool_dict in tool_list:
                tool_name = mock_tool_dict["name"]
                if tool_name in self.function_map:
                    logger.warning(f'Repeatedly adding tool {tool_name}, will use the newest tool in function list')
                # Use MockToolWrapper to wrap dictionary, providing unified interface
                self.function_map[tool_name] = MockToolWrapper(mock_tool_dict, server_description)
